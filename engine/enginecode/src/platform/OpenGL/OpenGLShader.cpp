...
namespace Engine {
	...
	{
		enum Region { None = -1, Vertex = 0, Fragment, Geometry, TessellationControl, TessallationEvalution, Compute } ;
		std::string line;
		...
		if (handle.is_open())
		{
			while (getline(handle, line)) 
			{
				...
				if (line.find("region TessallationEvalution") != std::string::npos) {region = Region::TessallationEvalution; continue;}
